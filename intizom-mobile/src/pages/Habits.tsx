...
import {
  ...
  Pencil, Trash2, Flame, Trophy, Check, Pause, Play,
} from 'lucide-react';
import {
...
import { HABIT_CATEGORIES, HABIT_COLORS, HABIT_ICONS } from '@/utils/constants';
...
function countAvailableDays(
  habit: HabitMonthRow, year: number, month: number,
  daysInMonth: number, todayDay: number, isCurrentMonth: boolean,
) {
  const first = getFirstActiveDay(habit, year, month);
  const last = Math.min(getLastActiveDay(habit, daysInMonth), isCurrentMonth ? todayDay : daysInMonth);
...
function HabitForm({ onSubmit, loading, defaultValues }: {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { category: 'other', color: '#0a9090', icon: '🎯', ...defaultValues },
  });
  const selectedColor = watch('color');
  const selectedIcon = watch('icon');

  return (
    <form onSubmit={handleSubmit((d) => onSubmit({ ...d, frequency: 'daily' }))} className="space-y-4">
      ...
      <div className="flex flex-col gap-2">
        <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Belgi</label>
        <div className="flex flex-wrap gap-2">
          {HABIT_ICONS.map((icon) => (
            <button key={icon} type="button" onClick={() => setValue('icon', icon)}
              className={cn('w-9 h-9 text-lg rounded-xl flex items-center justify-center transition-all',
                selectedIcon === icon ? 'ring-2 scale-110' : 'hover:bg-gray-100 dark:hover:bg-gray-800')}
    ...
    >
      ...
      <div className="flex items-center gap-3">
          ...
          {isPaused ? (
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="flex items-center gap-0.5 text-xs text-orange-500">
                <Flame size={11} /> {habit.streak} kun
              </span>
              <span className="flex items-center gap-0.5 text-xs text-yellow-500">
                <Trophy size={11} /> {habit.longestStreak}
              </span>
            </div>
          )}
        </div>
        ...
        <button
          ...
          className={cn(
            'w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-200 active:scale-90',
        ...
        >
          {todayDone
            ...
            : <div className="w-3 h-3 rounded-full" style={{ backgroundColor: habit.color + '60' }} />
          }
        </button>
      </div>

      {/* Dots row — month history */}
      {dots.length > 0 && (
        <div className="flex items-center gap-[3px] mt-3 flex-wrap">
          {dots.map(({ d, done, isToday }) => (
            <div
              key={d}
              className={cn('rounded-sm transition-all', isToday && 'ring-1 ring-offset-[1.5px]')}
              style={{
                width: 10,
                height: 10,
              ...
              return (
                <th key={day} className="py-3 px-0">
                  <div className={cn('flex flex-col items-center gap-0.5', isFuture && 'opacity-30')}>
                    ...
                    {isToday && <span className="w-1 h-1 rounded-full bg-[#0a9090]" />}
                  </div>
                </th>
              );
            })}
          ...
          {habits.map((habit) => {
            ...
            return (
              <tr key={habit.id} className="group border-b border-gray-100 dark:border-gray-800 last:border-0 hover:bg-gray-50/40 dark:hover:bg-gray-800/20 transition-colors">
                ...
                <td className="pl-4 pr-2 py-2">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm shrink-0"
                      style={{ backgroundColor: habit.color + (isPaused ? '10' : '20'), opacity: isPaused ? 0.5 : 1 }}>
                      {habit.icon}
                    </div>
                        ...
                        {isPaused ? (
                          ...
                          <>
                            ...
                            <span className="flex items-center gap-0.5 text-[10px] text-yellow-400">
                              <Trophy size={9} /> {habit.longestStreak}
                            </span>
                            <span className="text-[10px] text-gray-400">{habit.completedDays.length}k</span>
                          </>
                        )}
                      </div>
                    </div>
                ...
                {/* Day cells — only today clickable */}
                {Array.from({ length: daysInMonth }, (_, i) => {
                  const day = i + 1;
                  const isToday = isCurrentMonth && day === todayDay;
                  const isFuture = isCurrentMonth && day > todayDay;
                  ...
                  // Only TODAY is interactive
                  const disabled = !isToday || isFuture || isBeforeCreation || isAfterPause || isPaused;
                  ...
                  return (
                    <td key={day} className="py-2 px-0 text-center">
                      <button
                        ...
                        className={cn(
                          'mx-auto block transition-all duration-150',
                          disabled ? 'cursor-default' : 'cursor-pointer active:scale-90 hover:opacity-80',
                          isToday && !completed && !isPaused && 'ring-1 ring-offset-1',
                        )}
                        style={{
                          width: '78%', aspectRatio: '1',
                          minWidth: 13, maxWidth: 26,
                          borderRadius: 6,
                          backgroundColor: completed ? habit.color : 'transparent',
                          border: completed ? 'none' : isInactive ? 'none' : isToday ? `2px solid ${habit.color}80` : `1.5px solid ${habit.color}25`,
                          opacity: isInactive && !completed ? 0.15 : isPaused && !completed ? 0.2 : 1,
                        }}
                      >
                        {completed && isToday && (
                          <Check size={10} strokeWidth={3} color="white" style={{ margin: 'auto' }} />
                        )}
                        {completed && !isToday && (
                          <div style={{ width: '50%', height: '50%', borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.7)', margin: 'auto' }} />
                        )}
                      </button>
                    </td>
                  );
...
export function Habits() {
  ...
  // Stats
  const totalCells = habits.reduce((acc, h) =>
    acc + countAvailableDays(h, year, month, daysInMonth, todayDay, isCurrentMonth), 0);
  const completedCells = habits.reduce((acc, h) => {
    const first = getFirstActiveDay(h, year, month);
    const last = Math.min(getLastActiveDay(h, daysInMonth), isCurrentMonth ? todayDay : daysInMonth);
    return acc + h.completedDays.filter(d => d >= first && d <= last).length;
  }, 0);
  const overallPct = totalCells > 0 ? Math.round((completedCells / totalCells) * 100) : 0;
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div className="text-center min-w-[130px]">
            ...
            {habits.length > 0 && (
              <p className="text-xs text-gray-400">{completedCells}/{totalCells} · {overallPct}%</p>
            )}
          </div>
